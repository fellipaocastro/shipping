...
class Shipping(object):
    ...
    def is_valid_city_name(city_name):
        logger.info('CALL %s:%s STATIC METHOD' % ('Shipping', 'is_valid_city_name'))
        return re.match("^[a-zA-Z]+$", city_name) is not None

    @staticmethod
    def is_valid_number(number):
        logger.info('CALL %s:%s STATIC METHOD' % ('Shipping', 'is_valid_number'))
        return re.match("^\d+\.?\d*$", number) is not None

    @staticmethod
    def check_arguments_length(argv):
        logger.info('CALL %s:%s STATIC METHOD' % ('Shipping', 'check_arguments_length'))
        return True if len(argv) == 5 else False

    @staticmethod
    def check_arguments_type(argv):
        logger.info('CALL %s:%s STATIC METHOD' % ('Shipping', 'check_arguments_type'))
    ...
    def check_arguments(argv):
        logger.info('CALL %s:%s STATIC METHOD' % ('Shipping', 'check_arguments'))
...
def main():
    logger.info('CALL %s FUNCTION' % 'main')
    try:
